...
public class DiscordTextServiceImpl implements DiscordTextService {
    ...
    private static final String WEBHOOK_NAME = "GOOD";
    ...
    public void impersonateMember(Member member, TextChannel channel, String message) {
        channel.retrieveWebhooks().queue(webhooks -> {
            ...
            if (targetWebhook == null) {
                // 沒有就建立一個
                channel.createWebhook(WEBHOOK_NAME).queue(webhook -> doSend(webhook, member, message));
            } else {
                // 有就直接用
                doSend(targetWebhook, member, message);
            }
        });
    }
    ...
    private void doSend(Webhook webhook, Member member, String message) {
